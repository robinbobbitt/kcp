...
// TODO - Does this belong here or do I want it in a one-time-only spot like an init() or pass to the reconciler?
...
func init() {
	...
	// TODO - add extensions too for customresourcedefinition? and should this be that other oone generic whatever?
	openAPIDefinitionsGetters := []common.GetOpenAPIDefinitions{generatedopenapi.GetOpenAPIDefinitions}

	if apis, err := internalapis.CreateAPIResourceSchemas(schemes, openAPIDefinitionsGetters, KCPInternalAPIs...); err != nil {
		panic(err)
	} else {
		syncerSchemas = apis
	}
...
var KCPInternalAPIs = []internalapis.InternalAPI{
	{
